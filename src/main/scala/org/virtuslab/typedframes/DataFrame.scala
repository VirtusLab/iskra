...
object DataFrame:
  ...
  extension (df: UntypedDataFrame)
    transparent inline def typed[A](using encoder: DataType.StructEncoder[A]): DataFrame[?] = ${ typedImpl('df, 'encoder) } // TODO: Check schema at runtime? Check if names of columns match?

  private def typedImpl[A : Type](df: Expr[UntypedDataFrame], encoder: Expr[DataType.StructEncoder[A]])(using Quotes) =
    encoder match
